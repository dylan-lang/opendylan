...
define method add-transition
  ...
  if (table[code])
    error("input %= transitions to both %= and %=", on, table[code], new-state);
  else
    table[code] := new-state;
  end if;
end method add-transition;

define method add-transition
    (table :: <simple-object-vector>, on :: <byte-string>, new-state :: <symbol>)
 => ()
    ...
    if (range)
      if (last)
        for (i :: <integer> from as(<integer>, last) + 1 to as(<integer>, char))
          add-transition(table, as(<character>, i), new-state);
        end for;
        last := #f;
      else
