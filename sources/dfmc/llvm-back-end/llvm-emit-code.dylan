...
define method emit-code-aux
 ...
 => ();
  ...
  emit-lambda-body(back-end, module, o, function-type,
                   arguments, extra, calling-convention-parameters,
                   calling-convention, linkage, section);
end method;

define method emit-code-aux
    (back-end :: <llvm-back-end>, module :: <llvm-module>,
     o :: <&iep>, fun :: <&c-callable-function>, init?)
 => ();
  let function-type = llvm-c-function-type(back-end, fun);
  ...
  let linkage
    = if (fun.c-function-name) #"external" else #"internal" end;
...
define method emit-lambda-body
    (back-end :: <llvm-back-end>, module :: <llvm-module>, o :: <&iep>,
     function-type :: <llvm-type>,
     arguments :: <sequence>, extra :: <sequence>,
     calling-convention-parameters :: <sequence>,
