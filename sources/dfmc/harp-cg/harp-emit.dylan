  ...
  if (#t)
    ...
    for (o in user-init-code, count from 0)
      user-init-names[count]
        := ins--constant-ref(back-end,
                             format-to-string("%s_%d", user-name, count));
    end for;
    ...
    emitted-name(as(<symbol>, user-name))
      := with-harp-init-emitter (back-end, user-name,
                                 harp-debug: harp-output?,
                                 export: #f)
...
define method emit-init-code-body
    (back-end :: <harp-back-end>, stream, code-literal,
     #key source-locator)
  let o = code-literal.^iep;

  with-harp-variables (back-end, prototype: back-end.cg-variables)
...
define method emit-system-init-code
    (back-end :: <harp-back-end>, stream, heap :: <model-heap>, name,
  ...
  local
    method emit-fixups (back-end)
      ...
      for (refs in heap.heap-load-bound-references)
        emit-symbol-fixups
          (back-end, stream, load-bound-referenced-object(refs.first),
           refs, resolve-ref);
      end for;
    end method;

  with-harp-init-emitter (back-end, name,
                          harp-debug: harp-output?,
                          export: #f)
    if (code?)
      if (code? = #"fixups")
...
define method emit-user-init-code
...
// Register external symbols for runtime primitives and dylan
// functions referenced out of Heap

define method register-extern
    (back-end :: <harp-back-end>, object :: <runtime-object>,
     #key reference)
...
define method register-extern
    (back-end :: <harp-back-end>, object :: <constant-reference>,
     #key reference)
 => (reference)
...
// GNU Linker Support -- cache and dump imports per used library to
// .import files
          ...
          if (library == #"runtime")
            ...
            let internal-library-name =
              as-lowercase(as(<string>,
                              library.library-description-emit-name));
            values(internal-library-name,
                   apply(settings-executable,
                         library.library-description-build-settings)
  ...
  for (reference :: <constant-reference> in lambda-externals)
	...
	<&iep> =>
	  unless (object.function.lambda-runtime-function?)
            values(emit-name(back-end, #f, object),
...
define method emit-cg-name
 ...
 => (name :: type-union(<string>, <constant-reference>));
  if (*emitting-data?*)
...
define sideways method emit-name-internal
    (back-end :: <harp-back-end>, stream, o :: <&deferred-iep>)
 => (object :: <runtime-object>)
  let name = emit-iep-name(back-end, stream, o.function);
  entry-point-reference(name)
end method;

define function round-up-to-mod
    (int :: <integer>, modulus :: <integer>)
 => (i :: <integer>);
...
define method emit-symbol-fixups
  ...
  ins--move(back-end, object-ref,
            emit-fixup-reference(back-end, stream, object));
  let fixup
    = emit-resolve-symbol-call(back-end, stream, object-ref, resolve-ref);
...
define method emit-resolve-symbol-call
    (back-end :: <harp-back-end>, stream, object-ref, resolve-ref)
 => (fixedup-ref);
...
define method emit-fixup
 ...
 => ();
  ins--move(back-end,
            emit-fixup-reference(back-end, stream,
...
define method imported-object?
 ...
 => (import? :: <boolean>);
  ~ *compiling-dylan?*
    & library-imported-object?(current-library-description(), object)
end;

define method imported-object?
    (back-end :: <harp-back-end>, object :: <module-binding>)
 => (import? :: <boolean>);
  ~ *compiling-dylan?*
...
define method imported-object?
    (back-end :: <harp-back-end>, object :: <symbol>)
 => (import? :: <boolean>)
  case
    *compiling-dylan?* => #f;
...
define method imported-object?
    (back-end :: <harp-back-end>, object :: <&c-variable>)
 => (import? :: <boolean>);
  object.dll-import?
end;
...
define method imported-object?
    (back-end :: <harp-back-end>, object :: <string>)
 => (import? :: <boolean>);
  if (object = "")
    ~ *compiling-dylan?*
  end if
...
define method imported-object?
    (back-end :: <harp-back-end>, object :: <&shared-entry-point>)
 => (import? :: <boolean>);
  #f
end;

define method imported-object?
    (back-end :: <harp-back-end>, object :: <&engine-node-ep>)
 => (import? :: <boolean>);
  #f
end;
...
// Make appropriate unique model-objects for some models for which
// there is a many-to-one mapping to names

define method apropo-model-object (o :: <vector>) => (o);
...
define method apropo-model-object (o :: <byte-string>) => (o);
  ...
  if (*emitting-data?*)
    ...
    make-string-model-object(o)
  end if
end method;

define method apropo-model-object (o :: <&raw-type>) => (o);
  raw-type-marker()
end method;

define method apropo-model-object (o :: <uninterned-symbol>) => (o);
  as(<symbol>, o.symbol-name)
end method;

define method apropo-model-object (o :: <symbol>) => (o);
  o
end method;

define method apropo-model-object (o :: <&kernel-ep>) => (o);
  o.emitted-name
end method;

define method apropo-model-object (o :: <&keyword-method-mep>) => (o);
  o.emitted-name
end method;

define method apropo-model-object (o :: <&deferred-iep>) => (o);
  o.emitted-name
end method;

define method apropo-model-object (o :: <&c-function>) => (o);
  if (*emitting-data?*)
...
define method apropo-model-object (o) => (o);
  o
end method;
  ...
  let name
    ...
    | begin
        ...
        let import? =
          if (interactor?)
            #f;
          elseif (supplied?(import?))
            import?;
          else
            imported-object?(back-end, object);
          end if;
...
define function op--dylan-constant-ref
    (back-end :: <harp-back-end>, dylan-object :: <dylan-object>)
 => (ref);
  let object = dylan-object.dylan-model-object;
  let dylan-library? = *compiling-dylan?*;
  let (ref, found?)
    = op--constant-ref(back-end, object, import?: ~ dylan-library?);
  let already-seen?
    = case
        found? => #t;
        dylan-library? =>
...
define function register-dylan-code-models () => ();
  ...
  dylan-model-object($dylan-resolve-symbol-iep)
    := ^iep(dylan-value($symbol-fixup-name));
  dylan-model-object($dylan-unbound-instance-slot-iep)
    := ^iep(dylan-value(#"unbound-instance-slot"));
  dylan-model-object($dylan-type-check-error)
