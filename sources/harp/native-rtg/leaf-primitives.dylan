...
define method op--optionals-mep-apply-unvectored-with-required
  ...
  with-harp (be)
    ...
    if (max-num-arg-regs = 1)
      ins--ld(be, arg0, stack, 4);        // get the first arg into arg0
      ins--move-return-address(be, #f, 4, #f); // and shuffle the return address
    else
...
define leaf runtime-primitive apply
  ...
  if-return-address()
    ...
    ins--move(be, dst, stack);           // Destination for copy
  end;
