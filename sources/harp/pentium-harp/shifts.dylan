...
define method do-the-shift 
    (be :: <pentium-back-end>, ins :: <integer>, dest, count :: <integer>)
  unless (zero?(count))
    emit(be, #xc1);
...
define method do-the-shift 
    (be :: <pentium-back-end>, ins :: <integer>, dest, count :: <object>)
...
define method double-shift-left
    (be :: <pentium-back-end>, low, high, count :: <integer>)
...
define method double-shift-left
    (be :: <pentium-back-end>, low, high, count :: <object>)
  harp-out (be) move(be, ecx, count) end;
  // first, optionally shift the low bits into the high bits
  if (high)
    emit(be, #x0f);
