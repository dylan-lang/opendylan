Module:       carbon-duim
Synopsis:     Macintosh dialog implementation
...
define sealed method make-top-level-window
    (frame :: <dialog-frame>, sheet :: <carbon-top-level-sheet-mixin>,
     resource-id == #f)
...
define sealed method map-mirror
    (_port :: <carbon-port>,
     sheet :: <carbon-top-level-sheet-mixin>, mirror :: <dialog-mirror>) => ()
...
define sealed method frame-wrapper
    (framem :: <carbon-frame-manager>, dialog :: <dialog-frame>,
...
define sealed method update-frame-layout
    (framem :: <carbon-frame-manager>, frame :: <dialog-frame>) => ()
...
define sealed method do-exit-dialog
    (framem :: <carbon-frame-manager>, dialog :: <dialog-frame>, #key destroy? = #t) => ()
  // Under MAC, we need to re-enable the owner before we dismiss
...
define sealed method do-notify-user
 ...
 => (ok? :: <boolean>, exit-type)
  ...
  let parent-control = mirror-control(sheet-mirror(owner));
  let (visual, colormap, depth) = xt/control-visual-specs(parent-control);
  let title
    = title | select (style)
		#"information"   => "Note";
		#"question"      => "Note";
  ...
  cleanup
    when (dialog)
      ...
      xt/XtDestroyControl(shell)
    end;
  end
end method do-notify-user;

define xm/xm-callback-function notifier-button-press-callback
    (control, client-data :: <callback-client-data>, call-data :: xm/<XmPushButtonCallbackStruct>)
  ignore(control);
...
define xm/xm-callback-function notifier-delete-window-callback
    (control, value-setter, call-data)
  ignore(control, call-data);
  value-setter(#"cancel")
end xm/xm-callback-function notifier-delete-window-callback;

define xm/xm-callback-function notifier-map-callback
    (control, client-data :: <callback-client-data>, call-data)
  ignore(call-data);
  let owner-control = client-data.%owner-control;
  let lx = client-data.%x;
  let ly = client-data.%y;
  let (width, height) = xt/XtGetValues(control, #"width", #"height");
  let (rx, ry) = xt/XtTranslateCoords(owner-control, lx, ly);
  let (rwidth, rheight)
    = values(x/XWidthOfScreen(xt/XtScreen(control)), x/XHeightOfScreen(xt/XtScreen(control)));
  rx := rx - round/(width, 2);
  ry := ry - round/(height, 2);
  xt/XtSetValues(control,
...
define sealed method do-choose-file
 ...
 => (locator :: false-or(<string>), filter :: false-or(<integer>))
  ...
  let parent-control = mirror-control(sheet-mirror(owner));
  let (visual, colormap, depth) = xt/control-visual-specs(parent-control);
  let (directory, pattern) = carbon-directory-and-pattern(default, default-type);
  let shell-resources
    = vector(visual:, visual,
	     colormap:, colormap,
	     depth:, depth);
  ...
  block ()
    local method waiter () result end method,
	  method setter (value) result := value end method;
    shell  := xt/XtCreatePopupShell("ChooseFileShell", xm/<dialog-shell>, parent-control,
				   resources: shell-resources);
...
define xm/xm-callback-function choose-file-button-press-callback
    (control, client-data :: <callback-client-data>, call-data :: xm/<XmFileSelectionBoxCallbackStruct>)
  ignore(control);
...
define function carbon-directory-and-pattern
...
end function carbon-directory-and-pattern;
