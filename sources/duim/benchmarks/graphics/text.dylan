...
Synopsis:  Benchmarking of text drawing operations
...
define class <text-state> (<object>)
  slot %color = $red,
    init-keyword: color:;
  ...
  slot %filled? :: <boolean> = #f,
    init-keyword: filled?:;
end class <text-state>;

define class <text-info> (<object>)
  constant slot %state :: <text-state>,
...
define method initialize (info :: <text-info>, #key) => ()
  next-method();
...
define class <text-drawing-pane> (<drawing-pane>)
  slot %info :: false-or(<text-info>) = #f;
end class <text-drawing-pane>;

define method invalidate-text-info
    (pane :: <text-drawing-pane>) => ()
  let old-info = pane.%info;
  pane.%info := #f
//  pane.%info := old-info & make(object-class(old-info), count: old-info.%count)
end method invalidate-text-info;
...
define method do-allocate-space
    (pane :: <text-drawing-pane>, width :: <integer>, height :: <integer>)
 => ()
  next-method();
  invalidate-text-info(pane)
...
// options
define method draw-benchmark-text
    (pane :: <text-drawing-pane>, medium :: <medium>, region :: <region>) => ()
  ignore(region);
  let frame = sheet-frame(pane);
  let (width, height) = sheet-size(pane);
  let text-class = frame.%text-class;
  let info :: <text-info>
    = pane.%info
        | begin
            pane.%info := make(text-class,
                               count: frame.%count,
                               state: frame.%state,
...
define frame <text-drawing-benchmark-frame> (<simple-frame>)
  constant slot %state :: <text-state> = make(<text-state>);
  slot %count :: <integer> = 10,
    init-keyword: number:;
  slot %text-class :: subclass(<text-info>) = $default-drawing-operation;
...
end frame <text-drawing-benchmark-frame>;

define method switch-drawing-operation
    (gadget :: <gadget>) => ()
  let frame = sheet-frame(gadget);
  let pane = text-pane(frame);
  frame.%text-class := gadget-value(gadget);
  invalidate-text-info(pane);
  redraw-text-pane(frame)
...
define method redraw-text-pane
    (frame :: <text-drawing-benchmark-frame>) => ()
  let pane = frame.text-pane;
  clear-box*(pane, $everywhere);
  repaint-sheet(pane, $everywhere)
end method redraw-text-pane;

install-benchmark(<text-drawing-benchmark-frame>, "Text Drawing");
*/
