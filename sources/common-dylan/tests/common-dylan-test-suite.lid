Library:      common-dylan-test-suite
Synopsis:     Common Dylan library test suite
Author:       Andy Armstrong
Target-Type:  dll
Files: library
       specification
       variables
       classes
       functions
       macros
       streams
       byte-vector
       machine-words
       transcendentals
       regressions
       threads/simple-locks
       threads/recursive-locks
       threads/semaphores
       threads/notifications
       threads/threads
       threads/thread-variables
       threads/conditional-updates
       threads/threads-test-suite
       threads/threads-spec
       threads/synchronization-spec
       threads/locks-spec
       threads/semaphores-spec
       threads/exclusive-locks-spec
       threads/notifications-spec
       threads/misc-spec
Copyright:    Original Code is Copyright (c) 1995-2004 Functional Objects, Inc.
              All rights reserved.
License:      See License.txt in this distribution for details.
Warranty:     Distributed WITHOUT WARRANTY OF ANY KIND

