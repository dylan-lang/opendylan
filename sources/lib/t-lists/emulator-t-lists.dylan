...
// This is only debugged in the emulator.  <t-list> should be a subclass of
// <deque> (see commented out code below).  The current definition is an
// emulator artifact.

define open abstract primary class 
    <t-list> (<mutable-sequence>, <stretchy-collection>)
...
define method concatenate (the-t-list :: <t-list>, #rest more-sequences)
 => (new-t-list :: <t-list>);
  let new-t-list = shallow-copy(the-t-list);
  for (the-sequence in more-sequences)
    for (thing in the-sequence)
      push-last(new-t-list, thing);
    end for;
  end for;
  new-t-list
