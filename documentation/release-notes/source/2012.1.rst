...
* A new ``strings`` library replaces the ``string-extensions``
  library.  The latter will be removed in an upcoming release.  The
  new library provides a simpler API with all names exported from a
  single ``strings`` module.  `DEP-0004
