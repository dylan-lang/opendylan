...
* Any code needed by the library and module definitions.  For example, it is possible to
  ...
  section is defined in the ``dylan-user`` module, which is defined by the Dylan
  implementation, so there is no way to export the code for use in other modules.  More
  specifically, there is no way to use the code in the main body of the same source file.

  This is no different from multi-file projects but may be less intuitive due to the code
  being in the same file.  This is acceptable because it happens extremely rarely.
...
* *Exactly one* ``define module`` expression whose name matches the name in the
  ``Module`` header.  This module definition terminates the library and module definition
  section of the source file.  Other module definitions may precede it.
...
* The IDE could optionally be made to handle single-file libraries specially. However, it
  seems likely that there are deep assumptions built into the IDE about the meaning and
  behavior of ".dylan" and ".lid" files that would require more work than can be
  justified.  In contrast, on the command-line the context is quite clear, for example
  when invoking ``dylan-compiler -build foo.dylan``.

Model Implementation
====================

A simple way to implement this proposal would be to reverse the source file
transformations shown in the `Background`_ section, going from one file to three, and
then compiling the ``.lid`` file in the normal way.  A mechanism for mapping compiler
warning source code locations back to the original file would have to be devised.
