...
define method emit-computation (back-end :: <harp-back-end>, c :: <exit>) => ()
  if (c.entry-state.local-entry-state?)
    ...
    ins--push(back-end, emit-reference(back-end, #f, c.computation-value));
    ins--move(back-end, back-end.registers.reg-arg0, emit-reference(back-end, #f, c.entry-state));
    ins--call(back-end, $primitive-nlx.runtime-reference, back-end.registers.arguments-passed-in-registers);
...
define sideways method emit-object
  ...
  let bits = o.^raw-object-value.decode-single-float;
...
define sideways method emit-object
  ...
  let (low, high) = o.^raw-object-value.decode-double-float;
