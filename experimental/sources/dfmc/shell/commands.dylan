...
define dylan-shell-command 
  ...
  hidden? #f
  let locator = as(<locator>, project);
...
define dylan-shell-command
  ...
  with-project-location-handler (context)
	...
	(project, 
	 ...
	 abort-on-serious-warnings?: abort-on-serious-warnings, 
	 assembler-output?: assemble | *trace-asm?*, dfm-output?: dfm,
	 harp-output?: harp, debug-info?: ~ nodebug,
...
define dylan-shell-command 
  ...
  when (registry-path)
    os/environment-variable("FUNCTIONAL_DYLAN_USER_REGISTRIES") := registry-path;
  end;
