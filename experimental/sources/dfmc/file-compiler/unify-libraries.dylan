...
define function unify-project (#key config,
  ...
  when (config)
    ...
    let base-dir = override-locator(as(<locator>, config), name: #f);
    directory := if (supplied?(directory))
		   merge-locators(directory, base-dir)
		 else
		   base-dir;
		 end;
  end;
...
define function output-groups (dir :: <pathname>,
  ...
  let dir = as(<locator>, ensure-directory-name(dir));
  ...
  for (group in *grouper*.grouper-name-groups)
    ...
    when (registry)
      let registry-dir = merge-locators(registry, dir);
      let registry-loc = as(<locator>, ensure-directory-name(registry-dir));
      write-group-registry(group, registry-loc, lid-file);
    end;
  end;
end function;
...
define function write-group-registry (group :: <library-group>,
  ...
  let root = parent-directory(registry);
  ...
  with-open-file(s = override-locator(platform-registry, name: lib-name),
		 direction: #"output")
    format(s, "abstract://dylan/%s\n", abbreviate-locator(lid-file, root));
  end with-open-file;
end;
...
define function write-group-lid (group :: <library-group>,
  ...
  let lib-name = as-lowercase(as(<string>, group.group-library-name));
  let lid-file = override-locator(dir,
  ...
  with-open-file (stream = lid-file, direction: #"output")
	...
	let lib-file = override-locator(abbreviate-locator(lib-file, dir),
					type: #f);
...
define function combine-lid-properties (group :: <library-group>, dir)
  ...
  local method reloc-from (loc, file)
	  as(<string>, abbreviate-locator(merge-locators(file, loc), dir))
	end;
  ...
  local method new-files (files, loc, table)
	  choose(curry(new?, table),
		 map-as(<list>, curry(reloc-from, loc), files));
	end;
...
define function write-group-library (group :: <library-group>,
 ...
 => (lib-file :: false-or(<pathname>))
  ...
  unless (name == #"dylan") // no library file for dylan
    ...
    let lib-file = override-locator(dir,
...
define function compute-group-sealing (seal-all)
  ...
  for (group :: <library-group> in *grouper*.grouper-name-groups)
	...
	for (name in module.exported-names)
	  ...
	  if (form-compiler-open?(untracked-binding-definition(b, default: #f),
				  seal-all?)
...
define method form-compiler-open? (f :: <object>, all?) // #f or top-level-form
  #f
end;

define method form-compiler-open? (f :: <generic-definition>, all?)
  ~f.form-sealed? & (all? | member?(#"compiler-open", f.form-adjectives))
end;

define method form-compiler-open? (f :: <class-definition>, all?)
  ~f.form-sealed? & (all? | member?(#"compiler-open", f.form-adjectives))
end;
