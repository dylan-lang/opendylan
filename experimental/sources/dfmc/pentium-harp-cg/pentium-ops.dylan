...
define sideways method op--store-multiple-values-count(back-end :: <pentium-back-end>, reg) => ()
  if (reg.immediate??)
    if (reg == 1)
...
define sideways method op--bmvset(back-end :: <pentium-back-end>, tag :: <tag>) => ()
  ins--bmvset(back-end, tag);
end method;


define program-warning <pentium-harp-warning>
  slot condition-description, required-init-keyword: description:;
  format-string "%s";
  format-arguments description;
end;

define sideways method harp-warning (be :: <pentium-back-end>, #rest args)
  note(<pentium-harp-warning>,
