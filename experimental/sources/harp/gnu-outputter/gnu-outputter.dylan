...
define constant $linux-assemble-command-line =
  "as-dylan -L -d -o %s.o %s.s";
...
define method assemble-harp-outputter
  ...
  if (outputter.finished-outputting?)
    ...
    unless ($os-name == #"win32")
      let file-string = as(<string>, filename);
      let command-line =
	format-to-string($linux-assemble-command-line, file-string, file-string);
      run-application(command-line);
    end unless;
  end if;
end method;
...
// This produces assembly output for one function.
  ...
  while (code-index < total-len)
    ...
    if (label)
      ...
      code-index :=
	label-pos + truncate/(label.labelled-constant-size, code-item-increment);
    else
...
// Currently, we are relying on the Boehm GC on Linux; dump all data to the 
// standard data section

define method select-gnu-dylan-section
    (be :: <harp-back-end>, outputter :: <harp-linux-outputter>, section)
 => ()
  select-dylan-section(outputter, #".data", be.code-item-increment);
end method;
...
define method select-gnu-binary-section
    (outputter :: <harp-linux-outputter>, section,
     #rest keys)
 => ()
  apply(select-binary-section, outputter, ".data", keys);
end method;
...
define method output-function-type
  ...
  // ELF Outputter requires this to create correct Procedure Linkage Table
  // relocation type for functions
  let stream = outputter.destination;
...
define method output-data-footer
    (be :: <harp-back-end>, outputter :: <harp-linux-outputter>,
     ...
     #key model-object = unsupplied(),
     #all-keys) => ()
  // ELF Outputter requires this to create appropriate dynamic 
  // relocation type for Dylan data
  let section = outputter.current-section;
...
// Imports & exports are stubbed out for ELF; leave it all to the Linker
