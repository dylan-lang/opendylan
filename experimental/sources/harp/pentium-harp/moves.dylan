...
define method op--load-thread-local
    (be :: <pentium-back-end>, dest :: <register>, offset :: <integer>) => ()
  emit(be, fs.segment-prefix);
  harp-out(be)
    ld(be, dest, 0, offset);
  end harp-out;
...
define method op--tlb-base-register 
    (be :: <pentium-back-end>, dest :: <register>) => ()
  op--load-thread-local(be, dest, #x14);
end method;


// This is a temporary hack for Linux, until threads & thread-local
// storage is implemented in the runtime.
// At this time, only the master thread is supported.
// Nosa   Jun 25, 1999

define variable *thread-local-storage* = #f;

define function thread-local-storage(be :: <linux-pentium-back-end>)
 => (teb :: <constant-reference>)
  *thread-local-storage* |
    (*thread-local-storage* :=
       ins--constant-ref(be, raw-mangle(be, "%thread-local-storage")));
end;

define method op--store-thread-local
    (be :: <linux-pentium-back-end>, data, offset :: <integer>) => ()
  harp-out(be)
    st(be, data, thread-local-storage(be), offset);
  end harp-out;
end method;

define method op--load-thread-local
    (be :: <linux-pentium-back-end>, dest :: <register>, offset :: <integer>) => ()
  harp-out(be)
    ld(be, dest, thread-local-storage(be), offset);
  end harp-out;
end method;
