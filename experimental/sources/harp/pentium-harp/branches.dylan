...
define method emit-branch-sdi 
    (be :: <pentium-back-end>, opcode :: <integer>, tag :: <tag>)
...
define method emit-jmp-sdi (be :: <pentium-back-end>, tag :: <tag>)
...
define method emit-offset-sdi (be :: <pentium-back-end>, tag :: <tag>)
...
define method full-test 
      (be :: <pentium-back-end>, info :: <integer>) => (op :: <op>)
...
define method emit-pea-sdi 
    (be :: <pentium-back-end>, tag :: <tag>, offset :: <integer>)
...
with-ops-in pentium-instructions (load-nlx-address)
  c-preserved-destroys-fn := all-c-preserved-fn;  // not worth being smarter
  destroys-fn := constant-fn(pentium-allocatable-registers); 
  flag := #t;
end with-ops-in;

define pentium-template (load-nlx-address)
  pattern (be, tag, dest)
    harp-out (be) lea(be, tag, dest, 0) end;
end pentium-tamplate;
...
define method emit-scl 
    (be :: <pentium-back-end>, locator, named-registers :: <simple-object-vector>)
     => ()
