...
define method output-implicit-externals
    (backend :: <pentium-back-end>, outputter :: <harp-outputter>)
  output-external(backend, outputter, remove-optionals-runtime);
end method;

define method output-implicit-externals
    (backend :: <linux-pentium-back-end>, outputter :: <harp-outputter>)
  output-external(backend, outputter, remove-optionals-runtime);
  output-external(backend, outputter, thread-local-storage(backend));
end method;
...
define method file-extension-for-outputter-type
       (backend :: <pentium-back-end>, type == $coff-type$)
...
define method output-code-start
      (backend :: <pentium-back-end>, outputter :: <harp-coff-builder>) => ()
  next-method();
...
define method real-register-debug-info-enumeration
    (backend :: <pentium-back-end>, register :: <pentium-integer-register>)
    => (enumeration :: <integer>)
...
define method real-register-debug-info-enumeration
    (backend :: <pentium-back-end>, register :: <pentium-float-register>)
    => (enumeration :: <integer>)
...
define method real-register-from-debug-info-enumeration
    (backend :: <pentium-back-end>, enumeration :: <integer>)
...
define method spill-frame-pointer-offset
    (backend :: <pentium-back-end>, spill :: <spill>, with-frame :: <boolean>)
    => (offset :: <integer>)
  if (spill.arg-spill?)
...
define method spill-frame-pointer-offset
    (backend :: <pentium-back-end>, arg-number :: <integer>, with-frame :: <boolean>)
    => (offset :: <integer>)
...
define method output-compiled-lambda
    (be :: <pentium-back-end>, outputter :: <harp-binary-assembler-outputter>, lambda :: <fully-compiled-lambda>,
     #key)
    => ()
...
define method output-header
      (be :: <pentium-back-end>, outputter :: <harp-assembler-outputter>) => ()
  let asm-stream = outputter.outputter-stream;
  format(asm-stream, "\n.486\n");
...
define method output-external
    (be :: <pentium-back-end>, outputter :: <harp-assembler-outputter>, name :: <byte-string>,
     #key import?)
     => ()
  let asm-stream = outputter.outputter-stream;
...
define method output-comment
    (be :: <pentium-back-end>, outputter :: <harp-assembler-outputter>, comment :: <string>)
     => ()
  let asm-stream = outputter.outputter-stream;
  format(asm-stream, "\n;;;  %s   ;;;\n", comment);
  flag-asm-line-start(outputter);
