...
define method emit-one-byte (be :: <pentium-back-end>, x :: <integer>) 
  emit(be, one-byte(x));
end method;
...
define method emit-four-bytes (backend :: <pentium-back-end>, x :: <abstract-integer>)
  emit(backend,
...
define method emit-four-bytes (backend :: <pentium-back-end>, x :: <integer>)
  emit(backend,
       logand(x, #xff),
       logand(ash(x, -8), #xff),
       logand(ash(x, -16), #xff),
...
define method emit-two-bytes (backend :: <pentium-back-end>, x :: <integer>)
  emit(backend,
       logand(x, #xff),
       logand(ash(x, -8), #xff));
end method;
...
define method emit-reg-indir 
    (backend :: <pentium-back-end>, reg :: <real-register>, ex :: <integer>)
...
/// because the last case is so baroque, let's have a spill-operand:

define method signed-frame-pointer-offset 
    (backend :: <pentium-back-end>, x :: <gspill>) => (i :: <integer>)
...
define method signed-frame-pointer-offset 
    (backend :: <pentium-back-end>, x :: <nspill>) => (i :: <integer>)
  let state = backend.variables.vreg-state;
  -4 * (1 // one for the spill location itself
        + x.spill-offset
        + state.number-preserved
...
define method signed-frame-pointer-offset 
    (backend :: <pentium-back-end>, x :: <sfspill>) => (i :: <integer>)
  let state = backend.variables.vreg-state;
  -4 * (1 // one for the spill location itself
        + x.spill-offset
        + state.number-preserved
...
define method signed-frame-pointer-offset 
    (backend :: <pentium-back-end>, x :: <dfspill>) => (i :: <integer>)
  let state = backend.variables.vreg-state;
...
define method arg-offset 
    (backend :: <pentium-back-end>, operand :: <spill>, 
     #key with-frame = backend.variables.with-stack)
    => (i :: <integer>)
   4 * (if (with-frame) 2 else 1 end
...
define method arg-offset-from-arg-number
    (backend :: <pentium-back-end>, operand :: <integer>, 
     #key with-frame = backend.variables.with-stack)
    => (i :: <integer>)
   4 * (if (with-frame) 2 else 1 end
        + operand
...
define method emit-spill-operand 
    (backend :: <pentium-back-end>, spill :: <spill>, ex :: <integer>)
...
define method emit-reg-offset
   (backend :: <pentium-back-end>, reg :: <real-register>,
    offset == 0, ex :: <integer>)
...
define method emit-reg-constant-offset 
    (backend :: <pentium-back-end>, offset, ex :: <integer>)
  emit(backend, mod00 + ex + 5);
  emit-immediate-constant(backend, offset);
end method;
...
define method emit-m-c-spill-dest 
    (backend :: <pentium-back-end>, x, extension :: <integer>)
...
define method emit-immediate-constant
    (backend :: <pentium-back-end>, x :: <abstract-integer>)
  emit-four-bytes(backend, x);
end method;

define method emit-immediate-constant
    (backend :: <pentium-back-end>, x :: <constant-reference>)
...
define method xcg
    (backend :: <pentium-back-end>, one == eax, two :: <real-register>)
  emit(backend, #x90 + two.real-register-number);
end method;

define method xcg
    (backend :: <pentium-back-end>, one :: <real-register>, two == eax)
...
define method code-item-increment 
     (backend :: <pentium-back-end>) => (i :: <integer>)
  1;
end method;
...
define method runtime-reference 
  ...
  make(<constant-reference>, 
       refers-to: name, ///  old version ... concatenate("_z_", name),
