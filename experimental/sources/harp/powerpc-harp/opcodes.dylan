...
define macro with-ppc-op-hi-lo
  { with-ppc-op-hi-lo (?hi-var:name, ?lo-var:name, ?body:expression,
		       #key ?op:expression,
			    ?r1:expression, ?r2:expression, ?r3:expression, ?r4:expression,
			    ?d1:expression, ?d2:expression, ?d3:expression,
			    ?fsf:expression, ?low-part:expression, ?condition:expression, ?displacement:expression)
    } =>
  {
    begin
    ...
    let ?hi-var :: <integer> =
      mw-hi-part(?op) +
      (?r1 & reg-align(?r1, 21, #t)) +
      (?r2 & reg-align(?r2, 16, #t)) +
      (?d1 & field5-align(?d1, 21, #t)) +
      (?d2 & reg-align(?d2, 16, #t)) +
      (?fsf & field-align(lo-part(?fsf), 17, #t)) +
      (?condition & ash(?condition, -16));

    let ?lo-var :: <integer> =
      mw-lo-part(?op) +
      (?r3 & reg-align(?r3, 11, #f)) +
      (?r4 & reg-align(?r4, 6, #f)) +
      (?d3 & field5-align(?d3, 11, #f)) +
      (?low-part & lo-part(?low-part)) +
...
define macro emit-ppc
  { emit-ppc(?keys:*) } =>
  {
    with-ppc-op-hi-lo(high-part, low-part, emit-2(high-part, low-part), ?keys)
  }
end macro;

define macro sdi-ppc
  { sdi-ppc(?keys:*) } =>
  {
    with-ppc-op-hi-lo(high-part, low-part, pair(high-part, low-part), ?keys)
  }
end macro;
...
// define constant ov-cc = ash(#b001, 16);
...
// define constant bov-cc = brit-cc + ov-cc;  // branch if overflow (XER position)
// define constant bno-cc = brif-cc + ov-cc;  // branch if not overflow (XER)
