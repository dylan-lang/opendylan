...
define c-runtime-primitive dylan-call-in
  ...
  //           PLUS: 
  //              arg-count - size of arguments to call-in in words
  //              arg0      - function to be called inside MM trampoline
  // Behaviour:
  //  * Sets the runtime state to inside-dylan
  //  * Calls the call-in inside the trampoline with arg-count bytes of arguments copied
...
define method op--initialize-thread-with-gc
  ...
  with-harp (be)
    ...
    // Initialize FPU control word so floating-point exceptions
    // get signalled on this thread
    ins--init-control-word(be);
  end with-harp;
end method;
...
define shared init c-runtime-primitive init-dylan-data
...
define shared init c-runtime-primitive call-init-dylan
  // On entry:
  //    no args
  // On exit:
...
define shared init c-runtime-primitive dylan-initialize
  // On entry:
  //    no args
  // On exit:
  //    Returns 0
