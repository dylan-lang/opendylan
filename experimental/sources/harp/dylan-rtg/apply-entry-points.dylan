...
define method op--apply-xep-discriminating 
  ...
  with-harp (be)
    ...
    op--extend-stack-for-apply(be, vec, size, req-index);
    ...
    ins--add(be, stack, stack, 4);          // pop the stack
...
define open generic op--extend-stack-for-apply
    (be :: <harp-back-end>, vec, size, req-index);
...
define method op--extend-stack-for-apply
  ...
  with-harp (be)
    ...
    op--calculate-arg-count-for-apply(be, argc, req-index, nsize);
...
define method op--calculate-arg-count-for-apply
    (be :: <harp-back-end>, dest :: <register>, req-index, vec-size)
  let req-num = op--divide-by-4(be, req-index);
  // req-num is the number of passed parameters _apart_ from the vector
