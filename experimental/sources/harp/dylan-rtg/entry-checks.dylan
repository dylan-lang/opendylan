...
define method op--preserve-args-for-specializer-check
    (be :: <harp-back-end>, 
     value :: <register>, type :: <register>, specs :: <register>,
     #key spare1, spare2)
...
/*
  let extra-args :: <sequence> =
    op--move-to-registers(be, 1, specs, value, type, spare1, spare2);
...
define method op--load-specializer-saved-registers
  ...
  with-harp (be)
    frame frame;
    if (spare2) ins--ld(be, spare2, frame, -20) end;
    if (spare1) ins--ld(be, spare1, frame, -16) end;
    if (type)   ins--ld(be, type,   frame, -12) end;
    if (value)  ins--ld(be, value,  frame, -8)  end;
    if (specs)  ins--ld(be, specs,  frame, -4)  end;
  end with-harp;
end method;
...
define method op--store-specializer-saved-registers
  ...
  with-harp (be)
    frame frame;
    if (spare2) ins--st(be, spare2, frame, -20) end;
    if (spare1) ins--st(be, spare1, frame, -16) end;
    if (type)   ins--st(be, type,   frame, -12) end;
    if (value)  ins--st(be, value,  frame, -8)  end;
    if (specs)  ins--st(be, specs,  frame, -4)  end;
  end with-harp;
end method;
...
define method op--load-specializer-saved-arg-register
  ...
  with-harp (be)
    frame frame;
    ins--ld(be, be.registers.reg-machine-arguments[i],
	    frame, -20 - 4 * i);
  end with-harp;
end method;
...
define method op--specializer-checks-via-loop
  ...
  with-harp (be)
    ...
    op--preserve-args-for-specializer-check(be, dummy, dummy, specs, spare1: arg-num);
...
define method op--specializer-checks-in-line
  ...
  for (arg-num from 0 below required)
    with-harp (be)
      ...
      op--preserve-args-for-specializer-check(be, value, type, specs);
      with-preserved-arguments (function, mlist)
        op--primitive-instance?-setting-result(be, value, type);
        ins--beq(be, err-tag, result, dylan-false);
      end;
