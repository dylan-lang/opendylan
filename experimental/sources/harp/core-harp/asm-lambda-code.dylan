...
define open generic assemble-compiled-lambda
   (backend :: <harp-back-end>, print-info, public?, export?, source-locator)
   => (code :: <compiled-lambda>);


// The default method for assemble-compiled-lambda is for back ends which
// use byte-incremented code (i.e. for which code-item-increment is 1).
// The protocol is left open for other back ends.
...
define method assemble-compiled-lambda
  ...
  unless (backend.code-item-increment = 1)
    harp-error("Inappropriate method for assembling a lambda");
  end unless;
  ...
  let code = make(<byte-vector>, size: code-size); 
...
define open generic fill-compiled-lambda-code-and-labels
    (backend :: <harp-back-end>, 
     code :: <vector>, labels :: <vector>, debug-info :: <vector>)
    => ();


/// The default method for fill-compiled-lambda-code-and-labels fills in the
/// code and labels, for back ends with BYTE-ALIGNED code only. 
/// Back ends which are word aligned or must add a new method.
...
define method fill-compiled-lambda-code-and-labels
    (backend :: <harp-back-end>, 
     code :: <byte-vector>, labels :: <simple-object-vector>, debug-info :: <simple-object-vector>)
    => ();
...
/// before invoking this function - for efficiency's sake). Default methods
/// are provided for all known types of complex code item for byte aligned back ends,
/// For completeness, theres also an <integer> method - although integers should
/// have been handled at a higher level.


define open generic fill-code-for-item
...
define method fill-code-for-item
    ...
    => (new-code-index :: <integer>, 
        ...
        new-debug-info-index :: <integer>)
  code[code-index] := item;
  values(code-index + backend.code-item-increment, labels-index, debug-info-index);
end method;
...
define method fill-code-for-item
  ...
  values(code-index + item.labelled-constant-size, labels-index, debug-info-index + 1)
end method;
...
define method fill-code-for-item
  ...
  fill-code-with-word(backend, code-index + item.relative-offset, code, code-index);
  item.labelled-constant-index := code-index;  // set this to help the outputters
  labels[labels-index] := item;
  values(code-index + item.labelled-constant-size, labels-index + 1, debug-info-index)
end method;
...
define method fill-code-for-item
  ...
  fill-code-with-word(backend, item.labelled-constant-reference.cr-const-offset, code, code-index);
  item.labelled-constant-index := code-index;  // set this to help the outputters
  labels[labels-index] := item;
  values(code-index + item.labelled-constant-size, labels-index + 1, debug-info-index)
end method;
...
define method fill-code-for-item
  ...
  fill-code-with-word(backend, item.labelled-constant-reference.cr-const-offset, code, code-index + 1);
  item.labelled-constant-index := code-index;  // set this to help the outputters
  labels[labels-index] := item;
  values(code-index + item.labelled-constant-size, labels-index + 1, debug-info-index)
end method;
...
define method fill-code-with-word
  ...
  let (b0, b1, b2, b3) = split-word-into-bytes(val, #f);
  code[index]     := b0;
  code[index + 1] := b1;
  code[index + 2] := b2;
  code[index + 3] := b3;
...
/// The default method for compiled-lambda-size calculates the size in backend
/// specific code increments, but with no attempt to deal with split instructions 
/// (a la risc processor back ends in LispWorks). If any back end ever uses split
/// instructions, then a new method must be added.
...
define method code-vector-code-and-label-size 
  ...
  let code-inc = backend.code-item-increment;
  for (item in vec)
...
define method code-item-code-and-label-size 
    ...
    =>  (code-size :: <integer>, labels-size :: <integer>)
  values(backend.code-item-increment, 0);
end method;
...
define method code-item-code-and-label-size 
    ...
    =>  (code-size :: <integer>, labels-size :: <integer>)
  values(item.labelled-constant-size, 1);
end method;
...
define method compiled-lambda-referenced-data
    (backend :: <harp-back-end>, data :: <list>) => (res :: false-or(<byte-vector>))
  let len :: <integer> = data.size;
  if (len.zero?)
    #f; // No referenced data
  else
    let datavec = make(<byte-vector>, size: (len * 4));
    iterate process-data (index :: <integer> = 0, todo :: <list> = data)
      unless (todo.empty?)
        ...
        if (word)
          ...
          process-data(index + 4, todo.tail);
        else
          ...
          process-data(index + 8, next-todo.tail);
        end if;
      end unless;
    end iterate;
    datavec;
