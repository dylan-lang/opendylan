...
define function run-batch-compile 
  ...
  for (argument in arguments)
	...
	"assemble"           =>
	  case
	    param.empty?   => assemble? := #"mnemonic-assembler";
	    param = "masm" => assemble? := #"mnemonic-assembler";
	    param = "gnu"  => assemble? := #"gnu-outputter";
	    param = "elf"  =>
	      assemble? := #"linux-outputter";
	      current-powerpc-back-end() := <linux-powerpc-back-end>;
	    otherwise      =>
	      invalid-option-error(argument);
	  end case;
	"dfm"             => dfm?     := #t;
...
define function display-options () => ()
  ...
  display-banner(cl);
  let locator = as(<locator>, os/application-filename());
