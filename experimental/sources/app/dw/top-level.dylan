...
define function do-set-personal-root (personal-directory)
  os/environment-variable("FUNCTIONAL_DYLAN_USER_ROOT")
    := concatenate(personal-directory, "");
  os/environment-variable("FUNCTIONAL_DYLAN_USER_BUILD")
    := concatenate(personal-directory, "\\Build");
  os/environment-variable("FUNCTIONAL_DYLAN_USER_INSTALL")
    := concatenate(personal-directory, "");
  os/environment-variable("FUNCTIONAL_DYLAN_USER_SOURCES")
    := concatenate(personal-directory, "\\Sources");
  os/environment-variable("FUNCTIONAL_DYLAN_USER_REGISTRIES")
    := concatenate(personal-directory, "\\Sources\\registry");
end function;
...
define shell-command
  ...
  let personal-directory
    = os/environment-variable("FUNCTIONAL_DYLAN_USER_ROOT");
  format-out("FUNCTIONAL_DYLAN_USER_ROOT = %=\n", personal-directory);
end;

define command-argument system-directory;

define function do-set-system-root (system-directory)
  os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_ROOT")
    := concatenate(system-directory, "");
  os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_BUILD")
    := concatenate(system-directory, "\\Build");
  os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_INSTALL")
    := concatenate(system-directory, "");
  os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_SOURCES")
    := concatenate(system-directory, "\\Sources");
  os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_REGISTRIES")
    := concatenate(system-directory, "\\Sources\\registry");
end function;
...
define shell-command
  ...
  let system-directory
    = os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_ROOT");
  format-out("FUNCTIONAL_DYLAN_RELEASE_ROOT = %=\n", system-directory);
end;
...
define function do-set-target-platform (target-platform)
  let system-root = os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_ROOT");
  let user-root = os/environment-variable("FUNCTIONAL_DYLAN_USER_ROOT");
  if (system-root & user-root)
    os/environment-variable("FUNCTIONAL_DYLAN_RELEASE_INSTALL")
      := concatenate(system-root, "\\", target-platform);
    os/environment-variable("FUNCTIONAL_DYLAN_USER_INSTALL")
      := concatenate(user-root, "\\", target-platform);
    os/environment-variable("FUNCTIONAL_DYLAN_USER_BUILD")
      := concatenate(user-root, "\\", target-platform, "\\Build");
    close-all-projects();	//---*** NOTE: THIS IS OVERKILL, I think!
    target-platform-name() := target-platform;
    #t
  elseif (user-root)
    format-out("FUNCTIONAL_DYLAN_RELEASE_ROOT must be set to change platforms.\n");
    #f
  else
    format-out("FUNCTIONAL_DYLAN_USER_ROOT must be set to change platforms.\n");
    #f
  end
...
define shell-command
  ...
  documentation "prints the target platform\n"
                "\n"
  hidden? #t
  format-out("FUNCTIONAL_DYLAN_PLATFORM_NAME = %=\n", target-platform-name());
end;
...
define shell-command 
  ...
  hidden? #t
  heap-library
    (library, dfm-output?: dfm, harp-output?: harp, assembler-output?: assemble | *trace-asm?*);
end;
...
define shell-command 
  ...
  hidden? #t
  debug/link-library(library, dfm-output?: dfm, harp-output?: harp, assembler-output?: assemble | *trace-asm?*);
end;
...
define shell-command 
  ...
  hidden? #t
  link-glue
    (library, harp-output?: harp, assembler-output?: assemble | *trace-asm?*);
end;
...
define shell-command 
  ...
  if (*current-library-context*)
    compile-source-record
      (as-lowercase(as(<string>, filename)), *current-library-context*, force-compile?: force-compile, force-parse?: force-parse, dfm-output?: dfm, harp-output?: harp, assembler-output?: assemble | *trace-asm?*);
  else
...
define shell-command 
  ...
  if (*current-library-context*)
    recompile-source-record
      (as-lowercase(as(<string>, filename)), *current-library-context*, dfm-output?: dfm, harp-output?: harp, assembler-output?: assemble | *trace-asm?*);
  else
    format-out("\nSet current-library using:  in <library>   before attempting this command\n");
  end if;
end;
...
define shell-command 
  ...
  if (*current-library-context*)
    link-source-record
      (as-lowercase(as(<string>, filename)), *current-library-context*, dfm-output?: dfm, harp-output?: harp, assembler-ouptut?: assemble | *trace-asm?*);
  else
    format-out("\nSet current-library using:  in <library>   before attempting this command\n");
  end if;
end;
...
define shell-command 
  ...
  if (*current-library-context* & *current-file-context*)
    emit-source-record
      (as-lowercase(as(<string>, *current-file-context*)), *current-library-context*, dfm-output?: dfm, harp-output?: harp, assembler-output?: assemble | *trace-asm?*, form?: as-lowercase(as(<string>, formname)));
  else
    format-out("\nSet current-library using:  in <library>         before attempting this command");
...
define function project-info (context, project, #key warnings?)
  ...
  if(project.project-owners)
    map(curry(user-message, context, "owner project: %s\n"),
	project.project-owners);
  end;

  user-message(context, "Direct users:\n");
  ...
  if(compilation-context)
    user-message(context, "Database is: in memory (project has been parsed before)\n");
    user-message(context, "           : %s\n", if(current?) 
						 "describes current sources"
					       else
						 "is older then sources"
					       end);
    user-message(context, "           : is %s saved\n", if(saved?) "" else "not" end);
  else
...
define shell-command
  ...
  with-compiler-transaction
    let project = lookup-named-project(library);
    project-info(context, project, warnings?: warnings);
