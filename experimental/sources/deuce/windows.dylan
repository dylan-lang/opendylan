...
define open abstract class <basic-window> (<window>)
  ...
  sealed slot window-matching-string :: <byte-string> = make(<byte-string>, size: 1);
...
define method window-line-number
  ...
  | begin
      ...
      when (line)
	let index = bp->line-index(line-start(line), skip-test: #f);
...
define method window-total-lines
  ...
  | begin
      ...
      when (buffer)
	let total = count-lines(buffer, skip-test: #f);
...
define method window-note-policy-changed
  ...
  when (~old-policy
    ...
    // Don't redisplay immediately, in case previous-methods are also
    // going to do things requiring redisplay; leave that to the caller.
  end;
