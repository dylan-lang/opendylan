...
define variable $new-buffer-field-width :: <integer> = 250;
...
define sealed method new-buffer-dialog
 ...
 => (buffer-name :: false-or(<string>))
  ...
  with-frame-manager (framem)
    let buffer-name
      = make(<text-field>,
             min-width: $new-buffer-field-width,
	     value: "",
	     activate-callback: exit-dialog);
    let layout
      = make(<table-layout>,
	     x-spacing: 8, y-spacing: 2,
	     contents: vector(vector(make(<label>, label: "Name:"), 
				     buffer-name)));
    let dialog = make(<dialog-frame>,
		      title: title,
		      layout: layout,
		      input-focus: buffer-name,
		      mode: #"modal",
		      owner: sheet-frame(window));
    text-selection(buffer-name) := #t;
    let exit-status = start-dialog(dialog);
    when (exit-status)
      let (width, height) = sheet-size(buffer-name);
      ignore(height);
      $new-buffer-field-width := width
    end;
    exit-status & gadget-value(buffer-name)
  end
end method new-buffer-dialog;


// This can be tweaked by the user...
define variable $edit-definition-field-width :: <integer> = 250;
...
define sealed method edit-definition-dialog
 ...
 => (definition)
  ...
  with-frame-manager (framem)
    let definition-name
      = make(<text-field>,
             min-width: $edit-definition-field-width,
             value: name,
	     activate-callback: exit-dialog);
    let layout
      = make(<table-layout>,
	     x-spacing: 8, y-spacing: 2,
	     contents: vector(vector(make(<label>, label: "Definition:"), 
				     definition-name)));
    let dialog = make(<dialog-frame>,
		      title: title,
		      layout: layout,
		      input-focus: definition-name,
		      mode: #"modal",
		      owner: sheet-frame(window));
    text-selection(definition-name) := #t;
    let exit-status = start-dialog(dialog);
    when (exit-status)
      let (width, height) = sheet-size(definition-name);
      ignore(height);
      $edit-definition-field-width := width
    end;
    exit-status & gadget-value(definition-name)
  end
