...
define sealed method handle-event
  ...
  with-editor-state-bound (sheet)
	...
	when (dx ~= 0 | dy ~= 0)
	  if (abs(dx) < abs(dy))
	    let n = if (dy < top) -1 else 1 end;
	    let dy = scroll-n-lines(sheet, n);	// returns the amount actually scrolled
	    y := y + (if (n < 0) dy else -dy end)
	  else
	    let n = if (dx < left) -line-scroll-amount(sheet, #"horizontal")
	...
	when (bp & ~presentation & bp ~= window-point(sheet))
	  ...
	  // Move the point to extend the region
	  unless (bp = window-point(sheet))
	    move-point!(bp, window: sheet);
