  ...
  block (break)
	...
	select (direction)
	  #"forward" =>
		...
		scroll-n-lines(window, n);
		break();
	      otherwise =>
		scroll-n-lines-slowly(window, n);
		break();
	    end;
	  #"backward" =>
	    case
...
define command start-of-page (frame)
  ...
  let dline :: <display-line> = lines[0];
  let line  :: <basic-line>   = display-line-line(dline);
  move-point!(line, index: 0);
...
define command end-of-page (frame)
  ...
  let window :: <basic-window> = frame-window(frame);
  let lines = window-display-lines(window);
  let dline :: <display-line> = lines[window-n-display-lines(window) - 1];
  let line  :: <basic-line>   = display-line-line(dline);
  move-point!(line, index: 0);
  queue-redisplay(window, $display-point, centering: 1);
  frame-last-command-type(frame) := #"motion"
...
define command show-position (frame)
  ...
  let window :: <basic-window> = frame-window(frame);
  let buffer :: <basic-buffer> = frame-buffer(frame);
  let n-lines = count-lines(buffer);
  let line-no = count-lines(make-interval(interval-start-bp(buffer), point()));
  let percent = floor(100 * as(<single-float>, line-no + 1) / as(<single-float>, n-lines));
  let index   = bp-index(point()) + 1;
  let char    = bp-character(point());
  let code    = as(<integer>, char);
  if (code < #o40 | code > #o176)
    display-message(window, "Line %d of %d (%d%%), column %d, char code #o%o #x%x",
		    line-no, n-lines, percent, index, code, code);
  else
    display-message(window, "Line %d of %d (%d%%), column %d, char '%c' (#o%o #x%x)",
		    line-no, n-lines, percent, index, char, code, code);
  end;
  frame-last-command-type(frame) := #"display"
...
define command split-line (frame)
  ...
  with-change-recording (buffer, <paste-change-record>, start-bp: bp)
    ...
    insert-moving!(bp, '\n');
    move-point!(insert-moving!(bp, spaces))
  end;
  queue-redisplay(window, $display-blt, line: line, index: 1);
  frame-last-command-type(frame) := #"insert"
...
define command save-string-in-register (frame)
  ...
  let bp1 = point();
  let bp2 = mark();
  let string = if (bp2) as(<string>, make-interval(bp1, bp2)) else "" end;
  gethash($register-string-table, register) := string;
  frame-last-command-type(frame) := #"kill"
