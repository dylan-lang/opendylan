...
define constant $display-blt     :: <integer> = 4;	// insert or delete line(s) using 'bitblt'
define constant $display-text    :: <integer> = 5;	// any text in the window might have changed
...
define constant $display-partial-lines = #t;
define constant $recentering-fraction  = 0.0;
...
/*--- Old, very approximate method. Too often gets it wrong.
define sealed method recenter-window
  ...
  //--- This is a kludge, lines aren't all this height...
  let typical-line-height = font-height + vsp;
  ...
  case
    ...
    0.0 <= type & type <= 1.0 =>
      ...
      let n = max(floor(floor/(vheight, typical-line-height * type) - 1, 0);
      let line :: <basic-line>
	= bp-line(move-over-lines(line-start(line), -n, skip-test: #f));
      window-initial-line(window) := line;
  end;
...
define sealed method recenter-window
  ...
  let first-line
    = case
	type == #"top" =>
	  find-initial-display-line(window, line, line-number: 0);
	type == #"bottom" =>
	  find-initial-display-line(window, line, fraction: 1.0);
	type == #"center" =>
	  find-initial-display-line(window, line, fraction: 0.5);
	instance?(type, <integer>) =>
	  find-initial-display-line(window, line, line-number: type);
	0.0 <= type & type <= 1.0 =>
	  find-initial-display-line(window, line, fraction: type);
      end;
...
define method find-initial-display-line
 ...
 => (line :: <basic-line>)
  ...
  let (vwidth, vheight) = window-viewport-size(window);
  let vsp = window-line-spacing(window);
  local method find-line-for-space
	    (line :: <basic-line>, space :: <integer>)
	 => (line :: <basic-line>)
	  block (return)
	    while (space > 0)
	      if (line == first-line)
		...
		return()
	      ...
	      let previous-line
		= line-previous-in-buffer(line, buffer, skip-test: #f);
	      if (previous-line)
		line := previous-line
	      else
		return()
	  ...
	  line
	end method find-line-for-space;
  case
    fraction =>
      find-line-for-space(line, floor(vheight * fraction));
    line-number >= 0 =>
      block (return)
	while (line-number > 0)
	  ...
	  line := line-previous-in-buffer(line, buffer, skip-test: #f);
	  dec!(line-number)
	end
      end;
      line;
    otherwise =>
      let space-above :: <integer> = vheight;
      let next-line = line-next-in-buffer(line, buffer, skip-test: #f);
      let count :: <integer> = abs(line-number) - 1;
      block (return)
	while (next-line & count > 0)
	  let (line-width, line-height, line-baseline)
	    = line-size(line, mode, window);
	  ignore(line-width, line-baseline);
	  unless (zero?(line-height))
	    dec!(space-above, line-height);
	    dec!(space-above, vsp)
	  end;
	  if (space-above < 0)
	    return()
      ...
      if (space-above > 0)
	find-line-for-space(line, space-above)
      else
	line
      end;
  end
...
define sealed method do-redisplay-window
    (window :: <basic-window>, #key redisplay? = #f, move-point? = #t)
 => ()
  with-editor-state-bound (buffer = window)
	...
	block (return)
	  ...
	  when (redisplay? | degree >= $display-text)
	    redisplay-text(window, degree, redisplay?: redisplay?, move-point?: move-point?)
	  end;
	end;
...
define method redisplay-line
 ...
 => ()
  ...
  let (dx, dy)
    = values(if (marked?) 0 
	     else index->position(line, mode, window, index) - line-margin(line, mode, window) end,
	     display-line-y(dline));
...
define method redisplay-text
 ...
 => ()
  ...
  when (redisplay?)
    // We're going to redisplay anything anyway
    redisplay-y := #f
  end;
  when (redisplay-y)
  ...
  without-bounds-checks
	...
	display-line-tick(dline) := line-modification-tick(line);
	when (redisplay-y & dy < redisplay-y)
...
define sealed method update-caret-position
 ...
 => (visible? :: <boolean>)
  ...
  while (n < 2)
    ...
    let (x, y)
      = ~line-for-display-only?(bp-line(bp)) & bp->position(window, bp);
    case
      x & y =>
...
define method adjust-display-line
 ...
 => (line :: <display-line>)
  ...
  block (return)
    while (line-for-display-only?(display-line-line(line)))
      index := index + delta;
    ...
    without-bounds-checks
      local method measure-line (line :: <basic-line>)
		...
		display-line-tick(dline) := $minimum-integer
	      ...
	      if (line-y > max-y)
		if ($display-partial-lines | zero?(n-lines))
		  inc!(n-lines)
		end;
		return()
	      else
