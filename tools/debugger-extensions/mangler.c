...
demangle_dylan_char (char *name, int *src_i, char *demangled, int *dst_i)
{
  ...
  if (c == '_') {
    ...
    demangled[di++] = name[si++];
      ...
      else if (c == 'F') demangled[di++] = 'F'; /* squiggle */
      else
   ...
   boolean localp)
{
  ...
  if (localp || global_dylan_mangled_name_p(name)) {
    ...
    if (localp) {
      demangled[dst_i - 1] = '\0';  /* GET RID OF LOCAL SUFFIX */
    } else {
...
char *
mangle_local_dylan_name (char *name, char *mangled)
{
  int src_i = 0, dst_i = 0;

  mangle_dylan_string
    (name, &src_i, strlen(name), mangled, &dst_i);
