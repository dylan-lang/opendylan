...
define abstract primary class <external-file-accessor>
    (<external-stream-accessor>)
  slot file-size :: false-or(<integer>) = #f;
...
define method initialize
  ...
  // for #"output" streams.
  stream-position(stream) := stream.accessor.file-position;
  values();
...
define open generic accessor-file-size
  ( the-accessor :: <external-file-accessor> ) 
=> (the-size :: false-or(<integer>));

define method accessor-file-size
  ( the-accessor :: <external-file-accessor> ) 
=> (the-size :: false-or(<integer>));
  the-accessor.file-size
end method;
...
define function load-buffer
  ...
  if (desired-file-position ~= the-stream.accessor.file-position)
    accessor-set-file-position(the-stream.accessor, desired-file-position);
  end if;
  let (nread, eof?)
    = accessor-read-into!
...
define function force-buffer 
 ...
 => (the-buffer :: <buffer>)
  if (the-buffer.buffer-dirty?)
    ...
    if (new-file-position ~= the-stream.accessor.file-position)
      accessor-set-file-position(the-stream.accessor, new-file-position);
    end if;
...
define method stream-position-setter
 ...
 => (position :: <integer>)
  ...
  if ((position >= 0) & (position <= size-of-stream))
...
define method adjust-stream-position-from-start
  ...
  select (the-stream.stream-direction)
	...
	load-buffer(the-stream, the-buffer, new-buffer-position, 0, 
		    the-size);
