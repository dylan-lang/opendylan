...
define command start-keyboard-macro (frame)
  ...
  if (kbdmac & ~keyboard-macro-closed?(kbdmac))
    command-error("You are already defining a keyboard macro; cancelled")
  else
...
define sealed method abort-keyboard-macro-definition
    (frame :: <editor-state-mixin>) => (aborted? :: <boolean>)
  let kbdmac = frame-keyboard-macro(frame);
  if (kbdmac & ~keyboard-macro-closed?(kbdmac))
...
end method abort-keyboard-macro-definition;
