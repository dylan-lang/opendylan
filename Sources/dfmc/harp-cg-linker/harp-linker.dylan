...
// *default-debug-info?* controls whether debug-info is output with compiled lambdas
// by default.
//
...
define sideways method emit-library-records
    (back-end :: <harp-back-end>, ld :: <library-description>,
     #rest flags, 
     #key harp-output? = unsupplied(), assembler-output? = unsupplied(), cr, debug-info?,
     #all-keys) => ()
  if (cr)
...
define sideways method emit-library-record
    (back-end :: <harp-back-end>, cr :: <compilation-record>, ld :: <library-description>,
     #rest flags, 
     #key harp-output? = unsupplied(), assembler-output? = unsupplied(), force-link?, debug-info?,
     #all-keys) => ()
...
define sideways method link-and-download
    (back-end :: <harp-back-end>, il :: <interactive-layer>, runtime-context,
     #rest flags, 
     #key harp-output? = unsupplied(), assembler-output? = unsupplied(), debug-info? = #f,
     #all-keys)
 => (transaction-id)
  ...
  let component-name = as-lowercase(as(<byte-string>, ld.library-description-emit-name));
...
define method dll-imported-object? (back-end :: <harp-back-end>, object) => (res :: <boolean>)
  imported-object?(back-end, object) & (~ model-interactive?(object))
end method;

define method dll-imported-binding? (back-end :: <harp-back-end>, object) => (res :: <boolean>)
  library-imported-binding?(current-library-description(), object) & (~ binding-interactive?(object))
end method;


define method link-all 
    (back-end :: <harp-back-end>, stream, cr :: <compilation-record>, ld :: <library-description>,
            ...
            test: method(object, c-function)
	 	    ...
	 	    if (object.binding-name = c-fun.binding-name)
	 	      ...
	 	      | error("c-function %= has different calling convention "
			      "from its c-callable-function", object.binding-name)
                    end if
                  end method);
  locally-defined?
end method;
