...
#define	MV_GET_ELT(n) \
  (Preturn_values.count > (R(n)) ? Preturn_values.value[R(n)] : &KPfalseYinternalVdylan)
#define	MV_SET_ELT(n, t)	(Preturn_values.value[R(n)] = (t))
#define MV_SET_COUNT(n)		(Preturn_values.count = (R(n)))
#define MV_GET_COUNT()		(Preturn_values.count)

extern D MV_SPILL (void);
extern D MV_UNSPILL (D spill_t);
extern D MV_GET_REST_AT (D first);
extern D MV_SET_REST_AT (D v, D first);
...
extern D MAKE_CLOSURE(D, D, int, int, int, int, int, int, int, ...);
...
#define primitive_object_class(object) (((SOV*)object)->class)

#define primitive_slot_value(object, position) \
  (primitive_vector_element(object, I(R(position) - 1)))
#define primitive_slot_value_setter(new_value, object, position) \
  (primitive_vector_element_setter(new_value, object, I(R(position) - 1)))

#define primitive_repeated_slot_value(object, base_position, position) \
  (primitive_slot_value(object, I(R(base_position) + R(position))))
#define primitive_repeated_slot_value_setter(new_value, object, base_position, position) \
  (primitive_slot_value_setter \
     (new_value, object, I(R(base_position) + R(position))))
 ...
 */
#define primitive_unwrap_c_pointer(c_ptr) (*(((D*)(c_ptr))+1))

#define primitive_element(x, o, b) \
	AT(D,x,o,b)
#define primitive_element_setter(e, x, o, b) \
	AT_SETTER(D,e,x,o,b)
#define primitive_byte_element(x, o, b) \
	AT(DBCHR,x,o,b)
#define primitive_byte_element_setter(e, x, o, b) \
...
extern D primitive_integer_as_raw(D x);
