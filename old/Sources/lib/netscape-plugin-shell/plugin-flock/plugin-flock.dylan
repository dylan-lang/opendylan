...
define variable *flock-window* = #f;


define class <flock-plugin> (<plugin-instance>)
...
define c-struct <flock-plugin-saved-data>
end;

define c-pointer-type <flock-plugin-saved-data*>
  => <flock-plugin-saved-data>;
...
define method NPP-New (mime-type :: <byte-string>,
		       ...
		       parameters :: <list>,
		       saved :: <NPSavedData*>)
 => (instance :: <flock-plugin>);
  // could really do some more stuff with the parameters
  // and saved data
  // Note that c-instance: is a required-init-keyword.
  ignore(mime-type);
  ignore(parameters);
  make(<flock-plugin>, c-instance: c-instance, mode: mode, saved-data: saved)
end;

define method NPP-destroy (p :: <flock-plugin>) => (saved :: <NPSavedData*>);
  ignore(p);
  ...
  if(*flock-window*)
    destroy-frame(*flock-window*);
    *flock-window* := #f;
  end;
  let saved-data-package = as(<NPSavedData*>,
			      NPN-MemAlloc(size-of(<NPSavedData>)));
  let saved-data = as(<flock-plugin-saved-data*>,
		      NPN-MemAlloc(size-of(<flock-plugin-saved-data>)));
  saved-data-package.SavedData-buf := saved-data;
  saved-data-package.SavedData-len := size-of(<flock-plugin-saved-data>);
  saved-data-package
end;
  
define method NPP-SetWindow (dylan-instance :: <flock-plugin>,
			     np-window :: <NPWindow*>)
 => (err :: <integer>);
    //
  ...
  elseif (*flock-window*)
    ...
    let f = attach-flock(window-handle(np-window),
			    0, 0,
...
define method plugin-window-handler (dylan-instance :: <flock-plugin>,
				     hwnd :: <HWND>,
				     message :: <integer>,
				     wparam :: <integer>,
				     lparam :: <integer>)
...
define method NPP-NewStream (instance :: <flock-plugin>,
			     mime-type :: <string>,
			     stream :: <NPStream*>,
			     seekable? :: <boolean>,
			     stype :: <integer>)
...
define method NPP-WriteReady (instance :: <flock-plugin>,
			      np-stream :: <NPStream*>)
 => (ready-for :: <integer>);
  ignore(instance);
  ignore(np-stream);
...
define method NPP-Write (instance :: <flock-plugin>, np-stream :: <NPStream*>,
 ...
 */
define method NPP-DestroyStream (instance :: <flock-plugin>,
				 np-stream :: <NPStream*>,
				 reason :: <integer>)
 => (err :: <integer>);
  ignore(instance);
...
define method NPP-StreamAsFile (instance :: <flock-plugin>,
				np-stream :: <NPStream*>,
				fname :: <string>)
  => ();
  ignore(instance);
...
define method NPP-Print (instance :: <flock-plugin>,
			 print-info :: <NPPrint*>)
 => ();
  ignore(instance);
...
define method NPP-UrlNotify (dylan-instance :: <flock-plugin>,
			     dylan-url :: <string>,
			     reason :: <integer>,
