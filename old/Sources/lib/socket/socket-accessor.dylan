...
define open abstract class <socket-accessor> (<external-stream-accessor>)
  slot socket-descriptor :: false-or(<integer>), 
...
define method accessor-close
    (accessor :: <unix-socket-accessor>,
     #key abort? = #f, wait? = #t, direction = #"input-output")
 => (closed? :: <boolean>)
  let fd = accessor.socket-descriptor;
  if (fd)
    if ((c-socket-close(fd) < 0) & ~ abort?)
      socket-error("close");
    else
...
define method accessor-read-into!
 ...
 => (nread :: <integer>, end-of-file? :: <boolean>)
  ...
  if (accessor.socket-connection-eof?)
    ...
    let nread = socket-read(accessor.socket-descriptor,
			    as(<vector>, buffer), offset, count);
    if (nread < 0) // Error
      accessor.socket-connection-eof? := #t;
      socket-error("read")
    else // Might have more data, check for EOF (nread == 0)
      if( nread == 0)
	accessor.socket-connection-eof? := #t;
      end;
...
define method accessor-write-from
 ...
 => (nwritten :: <integer>, end-of-file? :: <boolean>)
  ...
  let remaining = count;
  while (remaining > 0)
    let nwritten = socket-write (accessor.socket-descriptor,
			       as(<vector>, buffer), 
